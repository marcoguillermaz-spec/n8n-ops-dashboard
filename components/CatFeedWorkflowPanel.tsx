...
export default function CatFeedWorkflowPanel() {
  ...
  const fetchWorkflow = useCallback(async () => {
    try {
      ...
      const catFeed = data.find((w) => w.label === 'Catalogue Feed');
      setWorkflow(catFeed ?? null);
      setError('');
    } catch (err: unknown) {
  ...
  // Fetch executions when workflow is loaded and panel is expanded
  useEffect(() => {
  ...
  // Status indicator for collapsed header
        ...
        <div className="border-t border-gray-800 px-6 py-6 space-y-8">
          ...
          {!loadingWf && !workflow && !error && (
            <div className="py-8 text-center text-sm text-gray-500">
              Workflow Catalogue Feed non trovato.
            </div>
          )}
